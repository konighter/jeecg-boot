        ...
        <a-space direction="vertical" style="width: 100%;">
          ...
          <a-card title="基本信息">
            ...
            <a-row type="flex" align="top">
              <a-col :span="2">
                <a-avatar :size="100" icon="user" :src="model.avatar" />
              </a-col>
              <a-col :span="19" offset="1">

                <a-descriptions>
                  <h2 slot="title">{{ model.name }}</h2>
                  <a-descriptions-item label=""> {{ model.gender }} <a-divider type="vertical" /> {{model.age}} </a-descriptions-item>
                  <a-descriptions-item label="最近工作"> {{ model.job }} </a-descriptions-item>
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      //   this.model = Object.assign({}, this.$router.params.item)
      // }
      console.log("Name: ", this.itemId)
      this.handleSingDetail(this.itemId)
    },
    methods: {
      handleSingDetail(itemId) {
        getAction(this.url.queryById, {id: itemId}).then(resp => {
          if (resp.code == 200) {
            this.model = Object.assign({}, resp.result)
          }else {
            this.$message.warning(resp.message);
          }
        })

        if (typeof this.editAfter === 'function') this.editAfter()
      },
